...
func Fetch[M any](
	...
	if rep.StatusCode != http.StatusOK {
		return nil, newStatusError(rep)
	}

	// decode the body
	err = json.NewDecoder(rep.Body).Decode(model)
	if err != nil {
		return nil, MalformedResponseBody{fmt.Errorf(
	...
	if err != nil {
		return nil, MalformedResponseHeaders{err}
	}
...
func (rc *RawClient) FetchHead(
	...
	output := cache.NewExpirableData[int32](pages, expires)
	return &output, nil
}
...
func fetchAuthInner[A any](
	...
	// if it's not 200, return an error
	if rep.StatusCode != http.StatusOK {
		return newStatusError(rep)
	}

	err = json.NewDecoder(rep.Body).Decode(esiAuthRep)
	if err != nil {
		return MalformedResponseBody{fmt.Errorf(
			"error decoding response body as json: %w",
			err,
		)}
	}
...
func (rc *RawClient) FetchJWKSJson(
	ctx context.Context,
