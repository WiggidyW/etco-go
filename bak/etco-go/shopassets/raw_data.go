...
// I lied up there, this one is from 11/8/23
func (ua *unflattenedAssets) addEntries(entries []esi.AssetsEntry) {
	for _, entry := range entries {
		ua.addEntry(entry)
	}
}

func (ua *unflattenedAssets) addEntry(entry esi.AssetsEntry) {
...
func (ua *unflattenedAssets) addFlattenedAsset(fltAsset flattenedAsset) {
	...
	// increase quantity of the existing (or non-existing) shopasset
	locationMap[fltAsset.TypeId] += fltAsset.Quantity
}

// flattens and filters the inner assets, converting them to valid inventory.ShopAssets
func (ua *unflattenedAssets) flattenAndFilter() map[int64]map[int32]int64 {
